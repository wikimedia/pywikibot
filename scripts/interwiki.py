...
import catlib
from pywikibot import pagegenerators, catlib, config2 as config
    ...
    def namespaceMismatch(self, linkingPage, linkedPage, counter):
        ...
        if linkedPage in self.foundIn:
            ...
            if globalvar.autonomous:
                pywikibot.output(u"NOTE: Ignoring link from page %s in namespace %i to page %s in namespace %i." % (self.linkingPage, self.linkingPage.namespace(), linkedPage, linkedPage.namespace()))
                ...
                if preferredPage:
                    pywikibot.output(u"NOTE: Ignoring link from page %s in namespace %i to page %s in namespace %i because page %s in the correct namespace has already been found." % (self.linkingPage, self.linkingnPage.namespace(), linkedPage, linkedPage.namespace(), preferredPage))
                    return True
                else:
                    choice = pywikibot.inputChoice(u'WARNING: %s is in namespace %i, but %s is in namespace %i. Follow it anyway?' % (self.originPage, self.originPage.namespace(), linkedPage, linkedPage.namespace()), ['Yes', 'No', 'Add an alternative', 'give up'], ['y', 'n', 'a', 'g'])
                    if choice != 'y':
    ...
    def disambigMismatch(self, page, counter):
        ...
        if globalvar.autonomous:
            if self.originPage.isDisambig() and not page.isDisambig():
                pywikibot.output(u"NOTE: Ignoring link from disambiguation page %s to non-disambiguation %s" % (self.originPage, page))
                return (True, None)
            elif not self.originPage.isDisambig() and page.isDisambig():
                pywikibot.output(u"NOTE: Ignoring link from non-disambiguation page %s to disambiguation %s" % (self.originPage, page))
                return (True, None)
        else:
            choice = 'y'
            if self.originPage.isDisambig() and not page.isDisambig():
                ...
                if nondisambig:
                    ...
                    choice = pywikibot.inputChoice(u'WARNING: %s doesn\'t seem to be a disambiguation page, but %s is one. Follow it anyway?' % (self.originPage, page), ['Yes', 'No', 'Add an alternative', 'Give up'], ['y', 'n', 'a', 'g'])
            if choice == 'n':
                return (True, None)
            elif choice == 'a':
